...
while int_Rank < 100
    ...
    arysymdb_SymbolWeighting = sym(zeros(int_SampleNum, int_Rank, int_Rank));
...
% 主要目的:計算每個 symbol 的 weighting matrix (cellmtxdb_SymbolWeighting)
    ...
    for i = 1:int_SampleNum
        for j = 1:int_Rank-1
			% arydb_MemberQuery(s,x,y) = member_query([x s y])
    ...
    while(i < int_VariableNum && flag) 
        ...
        while(j < int_Rank && flag) % run through y
	    	...
	    	if(acc ~= member_query([vecsymdb_Sample(vecint_IndexCounterExample(1:i)) cellvecsymdb_SymbolY{j}]))
	        	int_Rank = int_Rank + 1;
	        	cellvecsymdb_SymbolX{int_Rank} = vecsymdb_Sample(vecint_IndexCounterExample(1:i-1));
	        	cellvecsymdb_SymbolY{int_Rank} = [vecsymdb_Sample(vecint_IndexCounterExample(i)) cellvecsymdb_SymbolY{j}];
	        	flag = false;
	    	end
        end
    end
